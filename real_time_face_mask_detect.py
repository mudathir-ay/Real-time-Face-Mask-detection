...
# Loading ready model
state_dict = torch.load('Final_FaceMask_detection_model_new.pt', map_location=torch.device('cpu'))
    ...
    for box in boxes:
     ...
     #face region from the frame
        ...
        with torch.no_grad():
            ...
            class_probabilties=torch.softmax(outputs,dim=1)
            mask_probability=class_probabilties[0,1].item()
            unrelevant, predicted = torch.max(outputs, 1)
            print("predicted is:",predicted)
            print ("unrelevant output is:",unrelevant)
            print ("class_probabilties is:",class_probabilties)
            print ("mask_probability output is:",mask_probability)
            threshold=0.5
